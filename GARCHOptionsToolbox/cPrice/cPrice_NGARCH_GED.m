% Computes call option price under the NGARCH(1,1)-GED model
% by Monte Carlo simulation method
function [cPrice] = cPrice_NGARCH_GED(S_t,K,h_0,h_t,rF,tau,param)

lambda = param(1);  a = param(2);   b = param(3);   c = param(4);   nu = param(5);

w = h_0*(1-b-a*(1+c^2)); % Variance targeting where h_0 denotes the long-run variance
...
for i = 1:tau
  ...
  simVariance(:,i+1) = w + b*simVariance(:,i) + a*simVariance(:,i).*(eta_t-c).^2;
