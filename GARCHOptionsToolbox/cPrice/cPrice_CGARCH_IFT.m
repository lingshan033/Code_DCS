...
% Code for Component GARCH Model
...
function [c] = cPrice_CGARCH_IFT(S_t,K,h_t,rF,tau,param)

t_Hk = @(u)imag(CF_CGARCH(u-1i,S_t,h_t,tau,param).*exp(-1i.*u.*log(K))./(1i*u+1))./u;
...
function [Psi] = CF_CGARCH(u,S_t,h_t,tau,param)
...
omega = param(5);   phi = param(6);     rho = param(7);
...
for t = 2:T
    ...
    B2Mat1(:,t) = rho*B2Mat1(:,t-1) - 0.5*u1 + (2*(alpha*gamma_1*B1Mat1(:,t-1) + phi*gamma_2*B2Mat1(:,t-1) - 0.5*u1).^2)./(1 - 2*alpha*B1Mat1(:,t-1) - 2*phi*B2Mat1(:,t-1));
end
