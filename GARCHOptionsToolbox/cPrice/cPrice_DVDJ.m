...
function [cPrice] = cPrice_DVDJ(S_t,K,h_z0,h_zt,rF,tau,param)

lambda_z = param(1);    lambda_y = param(2);    w_z = param(3);
b_z = param(4);     a_z = param(5);     c_z = param(6);     d_z = param(7);
e_z = param(8);     theta = param(9);   delta = param(10);  kappa = param(11);
...
simJumpIntensity(:,1) = h_z0*kappa*Pi;
...
for i = 1:tau
  ...
  simJumpIntensity(:,i+1) = simVariance(:,i+1)*kappa*Pi;
