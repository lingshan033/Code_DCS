% Computes call option price under the NGARCH(1,1)-GED(C) model
% by Monte Carlo simulation method
function [cPrice] = cPrice_NGARCH_GEDC(S_t,K,h_0,h_t,q_t,rF,tau,param)
...
for i = 1:tau
  eta_t = gedinv(normcdf(simRandNormal(:,i)-lambda/b_nu-(0.5*b_nu-1/(2*b_nu))*sqrt(simVariance(:,i))),nu);
  simLongVariance(:,i+1) = w + rho*simLongVariance(:,i) + phi*simVariance(:,i).*(eta_t.^2-1-2*gamma_2*eta_t);  
  simVariance(:,i+1) = simLongVariance(:,i+1) + beta*(simVariance(:,i)-simLongVariance(:,i)) + alpha*simVariance(:,i).*(eta_t.^2-1-2*gamma_1*eta_t);  
  eta_t1 = gedinv(normcdf(simRandNormal(:,i+1)-lambda/b_nu-(0.5*b_nu-1/(2*b_nu))*sqrt(simVariance(:,i+1))),nu);
  simRet = rF + lambda*sqrt(simVariance(:,i+1)) - 0.5*simVariance(:,i+1) + sqrt(simVariance(:,i+1)).*eta_t1;
